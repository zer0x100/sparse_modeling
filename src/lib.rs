...
mod tests {
    ...
    fn mp_1sample_test() {
        ...
        let input_data: Array1<f64> = rand_pulses_signal(100, 10, 1.0, 2.0).expect("can't generate signal");
        let matrix: Array2<f64> =
            generate::random((input_data.shape()[0] / 2, input_data.shape()[0]));
        let output_data = matrix.dot(&input_data);
    ...
    fn compare_mps_test() {
        ...
        for support_size in supp_sizes_range.clone() {
            println!("signals, whose support sizes are {}, are generated and test mps", support_size);
            let mut supp_dist = (
                support_size,
                [0.; 4],
            );
            ...
            for it in 0..sample_size {
                ...
                supp_dist.1[0] += support_distance(&input_signal, &threshold_alg_result).expect("failed to compute support distace");
                supp_dist.1[1] += support_distance(&input_signal, &wmp_result).expect("failed to compute support distace");
                supp_dist.1[2] += support_distance(&input_signal, &mp_result).expect("failed to compute support distace");
                supp_dist.1[3] += support_distance(&input_signal, &omp_result).expect("failed to compute support distace");

                l2_err.1[0] += l2_relative_err(&input_signal, &threshold_alg_result).expect("can't calucalate l2 error");
                l2_err.1[1] += l2_relative_err(&input_signal, &wmp_result).expect("can't calucalate l2 error");
                l2_err.1[2] += l2_relative_err(&input_signal, &mp_result).expect("can't calucalate l2 error");
                l2_err.1[3] += l2_relative_err(&input_signal, &omp_result).expect("can't calucalate l2 error");
            }
